...
features = cPickle.load(open('nn_features150.p', 'rb'))
val_features = cPickle.load(open('nn_val_features150.p', 'rb'))
test_features = cPickle.load(open('nn_test_features150.p', 'rb'))
labels = cPickle.load(open('labels150.p', 'rb'))

#to normalize submatrix which is not sparse
l = [i for i in range(143, features.shape[1])]
features = features[:,l]
val_features = val_features[:,l]
test_features = test_features[:,l]
mu = np.mean(features, axis=0)
std = np.mean(features, axis=0)

features = (features - mu) / std
val_features = (val_features - mu) / std
test_features = (test_features - mu) / std
...
def model(x):
    ...
    layer = slim.batch_norm(layer, scope='bn5')
    layer = slim.dropout(layer, dropout_rate, scope='dropout5')
    layer = slim.fully_connected(layer,n_hidden6, weights_initializer=tf.truncated_normal_initializer(stddev=0.01),
                               weights_regularizer=slim.l2_regularizer(reg_strength),scope='hidden6')
    layer = slim.batch_norm(layer, scope='bn6')
    layer = slim.dropout(layer, dropout_rate, scope='dropout6')
    layer = slim.fully_connected(layer,n_hidden7, weights_initializer=tf.truncated_normal_initializer(stddev=0.01),
                               weights_regularizer=slim.l2_regularizer(reg_strength),scope='hidden7')
    layer = slim.batch_norm(layer, scope='bn7')
    layer = slim.dropout(layer, dropout_rate, scope='dropout7')
...
decay_steps = int(features.shape[0]/batch_size) * num_epochs_per_decay
